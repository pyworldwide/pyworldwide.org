...
export default function CodeOfConductEnforcing() {
  const enforcingData = cocProcedures.enforcing;
      ...
      <main className="container mx-auto px-4 py-12">
        <section className="mb-20 prose lg:prose-lg max-w-4xl mx-auto">
          <div className="mb-10">
            ...
            <p className="mb-6 text-gray-700">{enforcingData.description}</p>
          </div>

          {enforcingData.sections.map((section, index) => (
            <div key={index} className="mb-10">
              ...
              {section.orderedList && (
                <ol className="list-decimal ml-6 mb-6 text-gray-700">
                  {section.orderedList.map((item, iIndex) => (
              ...
              {section.subsections && section.subsections.map((subsection, subIndex) => (
                <div key={`subsection-${subIndex}`} className="mb-6">
                  <h3 className="text-xl font-bold mb-3">{subsection.title}</h3>
                  {subsection.list && (
                    <ul className="list-disc ml-6 mb-4 text-gray-700">
                      {subsection.list.map((item, itemIndex) => (
                        <li key={`sub-item-${itemIndex}`} className="mb-2">{item}</li>
                      ))}
                    </ul>
                  )}
                </div>
